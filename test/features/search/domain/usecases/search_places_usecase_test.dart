...
class MockPlaceRepository extends Mock implements PlaceRepository {
  ...
  Future<List<SearchResult>> searchPlaces(
    ...
    return super.noSuchMethod(
      Invocation.method(#searchPlaces, [query], {#latitude: latitude, #longitude: longitude, #radius: radius}),
...
void main() {
  ...
  group('SearchPlacesUseCase', () {
    final tQuery = 'beer';
    final tLocation = Position(latitude: 0, longitude: 0, timestamp: DateTime.now(), accuracy: 0, altitude: 0, heading: 0, speed: 0, speedAccuracy: 0, isMocked: false, altitudeAccuracy: 0, headingAccuracy: 0);
    final tRadius = 500;
    ...
    test('should return a list of SearchResult when the call is successful', () async {
      // Arrange
      when(mockPlaceRepository.searchPlaces(
        tQuery,
      ...
      verify(mockPlaceRepository.searchPlaces(
        tQuery,
        latitude: tLocation.latitude, longitude: tLocation.longitude,
        radius: tRadius,
      )).called(1);
    ...
    test('should throw an exception when the call to repository is unsuccessful', () async {
      // Arrange
      when(mockPlaceRepository.searchPlaces(
        tQuery,
        latitude: tLocation.latitude, longitude: tLocation.longitude,
